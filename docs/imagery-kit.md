...
- **2026-08-05** — Finished the sweep: a bespoke scene on every remaining bare
  page. `/book` a calendar with a circled date; `/pricing` two cups with $250 and
  $375 price tags; `/promise` a trust shield with a check and real fruit;
  `/ledger` an open book with a money-in and money-out column plus a coin stack;
  `/press` a newspaper and a camera; `/404` a melted, sad cup in a puddle. That
  is every non-arcade page either carrying a real photo (home, about, grown-ups,
  the service-area pages) or a bespoke illustration. The girls are going to react
...
- **Next up**: the girls' reactions and redraws. The real-photo shot list (limes
  ...
  library (cherry, pineapple ring, strawberry, a slush drip, more Slushie poses)
  is the other easy win. `/ideas` is generated by `docs/board/generate.js` and has
  its own visual system, so leave it to that generator.
...
| `/pricing` | Bespoke scene (two cups, $250 + $375 tags) | Good |
| `/book` | Bespoke scene (calendar + cup) | Good |
| `/promise` | Bespoke scene (trust shield + fruit) | Good |
